...
window.electronAPI.on('xpider-ext-runtime-on-message', async (message) => {
    ...
    if (message.action === 'performHardwareMove') {
        ...
        // 드래그 완료 후 renderer에서 직접 '이 지역 검색' 클릭 + 결과 스크롤 수행
        // (content.js의 격리 환경 스크롤 실패를 보완하는 2차 방어선)
        const wv = getActiveWebview();
        if (wv && wv.getURL().includes('google.com/maps')) {
            // 1) '이 지역 검색' 버튼 자동 클릭 (2.5초 후)
            setTimeout(async () => {
                try {
                    await wv.executeJavaScript(`
(function() {
  // 오작동 방지를 위해 가장 확실한 상단 플로팅 버튼 클래스만 타겟팅
  const SELS = [
    'button.NlVald', // '이 지역 검색' 표준 클래스
    'button.X69Czc', // '이 지역 검색' 대체 클래스
  ...
  for (const s of SELS) {
    ...
    if (el && el.offsetParent !== null) {
      ...
      // 화면 상단 중앙 부근에 있는 버튼인지 확인 (평점 버튼 등과 구분)
      if (rect.top < 200) {
          ['mousedown','mouseup','click'].forEach(t => el.dispatchEvent(new MouseEvent(t, {bubbles:true})));
          console.log('[XPIDER-CLICK] Verified Search Button Clicked:', s);
          return 'CLICKED';
      }
    }
  }
  return 'NOT_FOUND';
            ...
            // 2) 검색 결과 로딩 후 스크롤 수행 (5초 후 - 결과 로딩 대기)
            setTimeout(async () => {
                await forceScrollGmapsResults(wv);
            }, 5000);
        }
    }
            ...
            if (isAct) {
                ...
                if (!wv.hasAttribute('data-drag-attached')) {
                    ...
                    wv.addEventListener('mouseup', (e) => {
                        ...
                        if (dist > 20 && wv.getURL().includes('google.com/maps')) {
