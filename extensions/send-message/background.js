...
// [XPIDER] XPIDER Browser 환경 감지
// Electron에서는 chrome.tabs.create가 지원되지 않으므로
// chrome.tabs.query + chrome.tabs.update 기반의 대체 로직을 사용합니다.
const IS_XPIDER = typeof chrome.tabs === 'undefined' || typeof chrome.tabs.create !== 'function';
if (IS_XPIDER) console.log('[XPIDER Bridge] XPIDER Browser detected. Using compatible mode.');
...
async function orchestrateSending(urlInput, template) {
    ...
    // [XPIDER] chrome.tabs.create 미지원 환경 대응
    // XPIDER에서는 현재 활성 탭(메인 webview)을 재사용합니다.
    let tab;
    if (IS_XPIDER) {
    ...
    const startInjection = (delay) => {
        ...
        injectionTimer = setTimeout(async () => {
            ...
            try {
                ...
                // [XPIDER] XPIDER에서는 manifest의 content_scripts로 자동 주입됨 → executeScript 생략
                if (!IS_XPIDER) {
