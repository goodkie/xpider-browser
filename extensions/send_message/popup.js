...
`;
    ...
    reader.onloadend = function() {
        ...
        // Use Chrome Downloads API to show 'Save As' dialog if available
        if (typeof chrome.downloads !== 'undefined' && chrome.downloads.download) {
            chrome.downloads.download({
                ...
                if (downloadId) {
                    await trackExportHistory(tpl, filename);
                }
            });
        } else {
            // Electron Fallback
            ...
            setTimeout(() => {
                trackExportHistory(tpl, filename);
            }, 500);
        }
    };
...
async function trackExportHistory(tpl, filename) {
    ...
    if (typeof updateTemplateDropdown === 'function') await updateTemplateDropdown();
