...
function createWindow() {
  ...
  mainWindow.webContents.on('did-finish-load', () => {
    ...
    mainWindow.webContents.send('app_language', 'en');
...
(function() {
        ...
        return JSON.stringify(Array.isArray(emails) ? emails : []);
                    ...
                    if (emails.length > 0) {
                        ...
                        if (existing.size > before || existingAll.size > extStorage.allEmailsList.length) {
                            ...
                            extStorage.allEmailsList = [...existingAll].sort(); // popup.js 연동
                            saveExtStorage();
                    ...
                    if (extId) {
                        webContents.getAllWebContents().forEach(w => {
                            try { w.send('xpider-renderer-update-badge', { count: emails.length, extId }); } catch(e) {}
                        });
                    }
                } catch(e) {
                    // ignore navigate error
                }
            }, 3000);
        } catch(e) {}
    };
    ...
    wc.on('did-navigate-in-page', () => runEmailExtractor(wc));
});
...
// ─── WEBVIEW EXECUTE: sidepanel이 활성 웹뷰에 JS를 실행하고 결과 반환 ──────
// content.js의 window.__xpiderQueue를 드레인하는 폴링에 사용됨
...
ipcMain.handle('record-download', async (event, data) => {
    // Relay to renderer UI to store in localStorage
    const win = BrowserWindow.fromWebContents(event.sender);
    if (win) {
        win.webContents.send('xpider-record-download', data);
    }
    return { success: true };
});
...
if (!extStorage.emailExtractor) {
    extStorage.emailExtractor = { emails: [] };
}

// Receive emails collected from content.js via webview → postMessage → main
...
// Execute JS in active webview to get current page emails
        ...
        const emails = await mainWindow.webContents.executeJavaScript(`
            (async function() {
                ...
                try {
                    ...
                    const already = await wv.executeJavaScript('!!window.__emailExtractorLoaded').catch(() => false);
                    if (!already) {
                        const fs = require('fs');
                        const path = require('path');
                        // content script will be injected by did-finish-load handler
                        // trigger a fresh scan
                        await wv.executeJavaScript('window.__emailExtractorLoaded = false;').catch(() => {});
                    }
                    // Get emails from content script
...
// Handle file download from extension popup (blob URL → actual file via dialog)
ipcMain.handle('xpider-download-file', async (event, data) => {
    try {
        ...
        // If content is provided directly (from popup.js XPIDER_SEND path)
        ...
        // If blob URL provided (from chrome.downloads shim)
        if (data.url && data.url.startsWith('blob:')) {
            // We can't read blob URLs in main process; let the webview handle it
...
ipcMain.handle('xpider-ext-runtime-send-message', async (event, { message }) => {
    ...
    // ── CLEAR DATA: 모든 레이어 완전 완전 리셋 ──
    if (message.action === 'clearData') {
        log.info('[ClearData] ★ FULL WIPE initiated via IPC');
        // 1. 메모리 리셋
        ...
        // 3. 파일에 즉시 기록 (scrapedData: [] 상태로 덮어쓰기)
        try {
            fs.writeFileSync(storagePath, JSON.stringify({ scrapedData: [] }, null, 2));
            log.info('[ClearData] ✅ File storage wiped. scrapedData = []');
        } catch(e) {
            log.error('[ClearData] File write failed:', e.message);
        }
        // 4. 모든 webContents에 스토리지 변경 브로드캐스트
...
async function startDeepSearchInMain(hl = 'en') {
    ...
    for (const lead of leads) {
        ...
        try {
            ...
            for (const cUrl of [...new Set(home.contactLinks||[])].slice(0,5)) {
