    ...
    for (let index = 0; index < scanCount; index++) {
        ...
        await runWithTimeout((async () => {
            if (isCancelled) return;
                ...
                await checkLockdown();
                // TEXT_LIST: 사용자가 확인할 수 있도록 탭을 화면에 표시
                let scan = await scanPageInBrowser(url2, 5000, bizName, isTextListSource);
                await checkPause();

                // ─────────────────────────────────────────────────
                // TEXT_LIST 전용 4단계 수집 흐름
                // ─────────────────────────────────────────────────
                if (isTextListSource) {
                    // ① Google 관가 웹사이트 검색 결과
                    if (scan.homepage) {
                        sendLog(`  ✅ ① 홈페이지 발견: ${scan.homepage}`);
                    } else {
                        sendLog(`  ⚠️ ① Google 검색에서 홈페이지 미발견 → 하위 대체 검색 시도...`);
                        // ② 홈페이지 미발견 시: 단순화 쿼리로 재시도
                        const gTld2 = { kr: 'co.kr', jp: 'co.jp' }[gl] || 'com';
                        const fallbackQ = encodeURIComponent(`${bizName} website`);
                        ...
                        if (scanFb.homepage) {
                            ...
                            sendLog(`  ✅ ② 대체 검색에서 홈페이지 발견: ${scan.homepage}`);
                        } else {
                            sendLog(`  ❌ 홈페이지를 찾지 못함. ${bizName} 스킵.`);
                        }
                    }

                    // ③ 홈페이지 발견 시 컨택트 페이지 탐색 및 수집 (탭 표시 + 완료 후 자동 닫기)
                    if (scan.homepage) {
                        sendLog(`  🔍 ③ 컨택트 페이지 탐색 시작: ${scan.homepage}`);
                        await sendStatusDetail(`[Stage 3] 컨택트 페이지 수집 중 [${index+1}/${scanCount}] ${bizName}`);
                        // showTab=true: 업체 웹사이트도 탭에 표시, 완료 후 scrapeBusinessWebsite 내부에서 닫힘
                        const webScan = await scrapeBusinessWebsite(scan.homepage, targetOption, hl, gl, 5000, true);
                        if (webScan) {
                            if (webScan.emails) { scan.emails = webScan.emails; sendLog(`    ✅ 이메일: ${webScan.emails}`); }
                            if (webScan.phone)  { scan.phone  = webScan.phone;  sendLog(`    ✅ 전화: ${webScan.phone}`); }
                            if (webScan.address && webScan.address !== '-') { scan.address = webScan.address; sendLog(`    ✅ 주소: ${webScan.address}`); }
                            if (webScan.sns && webScan.sns.length > 0) { scan.sns = webScan.sns; sendLog(`    ✅ SNS: ${webScan.sns.join(', ')}`); }
                        }
                    }
                    ...
                    sendLog(`  ✓ [${index+1}/${targets.length}] "${bizName}" 저장 완료.`);
...
// [v1.1.2] XPIDER 페이지 스캔: chrome.runtime.sendMessage를 통해 popup.js 릴레이 → ext-preload.js IPC → 메인 프로세스
// Service Worker에는 window가 없으므로, popup.js를 중계자로 사용합니다.
function xpiderScanPage(url, waitMs = 5000, showTab = false) {
    return new Promise((resolve) => {
        const reqId = 'scan_' + Date.now() + '_' + Math.random().toString(36).slice(2);
        const timeout = setTimeout(() => resolve(null), waitMs + 15000);
        chrome.runtime.sendMessage(
...
// [v1.1.2] scrapeBusinessWebsite - 4단계 수집 흐름:
// 1. 홈페이지 열기 → 2. 컨텍트 페이지 링크 탐색 → 3. 컨텍트 페이지에서 이메일/주소/SNS/전화 수집 → 4. 결과 반환
// [v1.1.2] scrapeBusinessWebsite - 완전 순차 처리 (탭 하나씩 열고 닫기)
// 1. 홈페이지 탭 열기 → 정보 + 컨텍트 링크 동시 추출 → 탭 닫기
// 2. 컨텍트 페이지 탭 열기 → 이메일/주소/SNS/전화 수집 → 탭 닫기
async function scrapeBusinessWebsite(url, targetOption = 'all', hl = 'en', gl = 'us', waitMs = 5000, showTab = false) {
    if (!url || !url.startsWith('http')) return null;
    try {
        sendLog(`  🌐 웹사이트 열기: ${url}`);

        // 홈페이지 + 컨텍트 링크를 한 번의 탭 방문으로 처리
        ...
        sendLog(`  📋 컨텍트 링크: ${contactLinks.length}개 | 이메일:${emails.size}개`);

        // 컨텍트 페이지 방문 (최대 2개, 완전 순차)
        ...
        sendLog(`  ✅ 완료 - 이메일:${[...emails].length} 전화:${phone?'Y':'N'} 주소:${address?'Y':'N'}`);
        return result;
    } catch(e) {
        console.error('[scrapeBusinessWebsite]', e.message);
        return null;
    }
}

// 홈페이지를 열어 비즈니스 정보 + 컨텍트 링크를 한 번에 추출 (탭 1번만 사용)
...
// IPC 방식 페이지 스캔 (Google/Bing 검색 결과용)
function xpiderScanPage(url, waitMs = 5000, showTab = false) {
    return new Promise((resolve) => {
        const reqId = 'scan_' + Date.now() + '_' + Math.random().toString(36).slice(2);
        const timeout = setTimeout(() => resolve(null), waitMs + 18000);
                ...
                await runWithTimeout((async () => {
                    ...
                    try {
                        ...
                        if (rawResults && rawResults.length > 0) {
                            ...
                            // [XPIDER BUG FIX #1] 'for (const t of targets)'가 번역함수 t를 덮어쓰는 치명적 버그 수정
                            ...
                            if (targets.length > 0 && !isCancelled) {
                                ...
                                // t는 번역함수 (getT()에서 받은 것) - 위의 for-of 루프로 덮어쓰이지 않음
